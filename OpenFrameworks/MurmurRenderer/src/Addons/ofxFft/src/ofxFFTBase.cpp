...
void ofxFFTBase::updateAudioData(ofxFFTData & audioData, float * dataNew) {
    ...
    for(int i=0; i<audioData.size; i++) {
        audioData.data[i] = dataNew[i]*audioData.volume;
    }
...
void ofxFFTBase::setVolume(float value) {
    
    value = (exp(value)-1)/(M_E-1);
    value = ofClamp(value, 0, 1);
    fftData.volume = value;
}

float ofxFFTBase::getVolume() {
    return fftData.volume;
}
