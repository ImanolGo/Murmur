...
void main(){
    ...
    vec2 shift;
    // shift.x = cnoise( vec3(pos*frequency, time * speed) )*amplitude;
    // shift.y = cnoise( vec3(pos*frequency, time * speed) )*amplitude;
    shift.x = cnoise( vec3(pos*0.005, time * 0.5 + 17.0) )*20.0;
    shift.y = cnoise( vec3(pos*0.005, time * 0.5 + 12.0) )*20.0;
    pos += shift;
